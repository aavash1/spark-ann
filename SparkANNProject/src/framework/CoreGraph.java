...
public class CoreGraph implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1981433410167942321L;
	...
	private Map<Integer, Node> m_nodesWithInforMap = new HashMap<>();
	private ArrayList<cEdge> m_edgesWithInfo = new ArrayList<cEdge>();
	private Map<Integer, cEdge> m_edgesWithInfoMap = new HashMap<>();
	...
	public void setNodesWithInfoMap(Map<Integer, Node> nodeMap) {
		this.m_nodesWithInforMap = nodeMap;
	}
	...
	public Node getNodeFromMap(int nodeId) {
		return m_nodesWithInforMap.get(nodeId);
	}
	...
	public boolean addEdge(int edgeId, int startNode, int endNode, double distance) {
		...
		newEdge.setLength(distance);
		m_edgesWithInfo.add(newEdge);
		m_edgesWithInfoMap.put(edgeId, newEdge);
		if (!m_adjancencyMap.get(startNode).containsKey(endNode)) {
			m_adjancencyMap.get(startNode).put(endNode, distance);
			m_adjancencyMap.get(endNode).put(startNode, distance);
			m_numEdges++;
	...
	public void setEdgesWithInfoMap(Map<Integer, cEdge> edgeMap) {
		this.m_edgesWithInfoMap = edgeMap;
	}

	public cEdge getEdgeFromMap(int edgeId) {
		return m_edgesWithInfoMap.get(edgeId);
	}
